...
class UserRepository implements UserRepositoryInterface
{
    ...
    public function getData($request)
    {
        ...
        $finalData = $primaryQuery->orderBy($orderBy, $order)->get();

        // Second method (end) with all selected in frontend

        // Get the total count of items (for pagination)
        $totalData = User::count(); // You can optimize this query if needed
        return ['data' => ['data' => $finalData, 'total' => $totalData, 'request' => $request->all()], 'message' => 'All user data.', 'status' => 200];
    }
